package com.tkdz.game;
import com.tkdz.map.GameMap;
import com.tkdz.map.MapTile;
import com.tkdz.tank.tank;
import com.tkdz.tank.EnemyTank;
import com.tkdz.tank.MyTank;
import com.tkdz.util.MusicUtil;
import com.tkdz.util.MyUtil;
import jdk.internal.org.objectweb.asm.tree.TryCatchBlockNode;
import org.omg.CORBA.PUBLIC_MEMBER;
import org.omg.CORBA.portable.ValueOutputStream;

import java.awt.*;
import java.awt.event.KeyAdapter;
import java.awt.event.KeyEvent;
import java.awt.event.WindowAdapter;
import java.awt.event.WindowEvent;
import java.awt.image.BufferedImage;
import java.util.ArrayList;
import java.util.List;

import static com.tkdz.util.Constant.*;
//游戏的主窗口类，所有的游戏展示的内容都要在该类中实现
public class GameFrame extends Frame implements Runnable {
    //第一次使用的时候加载，而不是类加载的时候加载
    private Image overImg = null;
    //定义一张和屏幕大小一致的图片
    private BufferedImage bufImg = new BufferedImage(FRAME_WIDTH, FRAME_HEIGHT, BufferedImage.TYPE_4BYTE_ABGR);
    //游戏状态
    private static int gameState;
    //菜单指向
    private static int menuIndex;
    //标题栏的高度
    public static int titleBarH;
    //定义坦克对象
    private static MyTank myTank;
    //敌人的坦克容器
    private static ArrayList<tank> enemies = new ArrayList<>();
    //地图相关的内容
    //用来记录本关卡产生了多少个敌人
    private static int bornEnemyCount;
    public static int killEnemyCount;
    public static GameMap gameMap = new GameMap();
    //开始过关动画
    public static int flashTime;
    public static final int RECT_WIDTH = 40;
    public static final int RECT_COUNT = FRAME_WIDTH/RECT_WIDTH+1;
    public static boolean isOpen = false;
    public static void startCrossLevel(){
        gameState = STATE_CROSS;
        flashTime = 0;
        isOpen = false;
    }
    //绘制过关动画
    public void drawCross(Graphics g){
        gameMap.drawBk(g);
        myTank.draw(g);
        gameMap.drawCover(g);
        g.setColor(Color.BLACK);
        //关闭百叶窗的效果
        if(!isOpen){
            for (int i = 0; i < RECT_COUNT; i++) {
                g.fillRect(i*RECT_WIDTH,0,flashTime,FRAME_HEIGHT);
            }
            //所有的叶片都关闭了
            if(flashTime++ - RECT_WIDTH >5){
                isOpen = true;
                //初始化下一个地图
                gameMap.initMap(LevelInfo.getInstance().getLevel()+1);
            }
            else{
                //开百叶窗的效果
                for (int i = 0; i < RECT_COUNT; i++) {
                    g.fillRect(i*RECT_WIDTH,0,flashTime,FRAME_HEIGHT);
                }
                if(flashTime--==0)
                {
                    startGame(LevelInfo.getInstance().getLevel());
                }
            }
        }
    }

    public GameFrame()//对窗口初始化
    {
        initFrame();
        initEventListener();
        //启动用于刷新窗口的线程
        new Thread(this).start();
    }

    //对游戏进行初始化
    private void initGame() {
        gameState = STATE_MENU;
    }

    private void initFrame()//初始化属性
    {
        setTitle(GAME_TITLE);
        //窗口大小
        setSize(FRAME_WIDTH, FRAME_HEIGHT);
        //设置窗口左上角的坐标
        setLocation(FRAME_X, FRAME_Y);
        setResizable(false);
        setVisible(true);//设置窗口可见
        //求标题栏的高度
        titleBarH = getInsets().top;
    }

    //初始化窗口的事件监听
    private void initEventListener() {
        addWindowListener(new WindowAdapter() {
            @Override
            //点击关闭按钮的时候，方法会被自动调用
            //给窗口注册监听事件
            public void windowClosing(WindowEvent e) {
                System.exit(0);
            }
        });
        //添加按键监听事件
        addKeyListener(new KeyAdapter() {
            //按键被按下时被回调的方法
            @Override
            public void keyPressed(KeyEvent e) {
                //不同的游戏状态给出不同的处理方法
                //被按下的键的键值
                int keyCode = e.getKeyCode();
                switch (gameState) {
                    case STATE_MENU:
                        keyPressedEventMenu(keyCode);
                        break;
                    case STATE_HELP:
                        keyPressedEventHelp(keyCode);
                        break;
                    case STATE_ABOUT:
                        keyPressedEventAbout(keyCode);
                        break;
                    case STATE_RUN:
                        keyPressedEventRun(keyCode);
                        break;
                    case STATE_LOSE:
                        keyPressedEventLose(keyCode);
                        break;
                    case STATE_WIN:
                        keyPressedEventWin(keyCode);
                        break;

                }
            }

            //按键松开时回调的内容
            @Override
            public void keyReleased(KeyEvent e) {
                //键盘松开时调用的方法
                int keyCode = e.getKeyCode();
                if (gameState == STATE_RUN) {
                    keyReleasedEventRun(keyCode);
                }
            }
        });
    }
    //游戏通关的按键处理
    private void keyPressedEventWin(int keyCode) {
        keyPressedEventLose(keyCode);
    }

    //按键松开的时候游戏中处理的方法
    private void keyReleasedEventRun(int m2) {
        switch (m2) {
            case KeyEvent.VK_UP:
            case KeyEvent.VK_W:
            case KeyEvent.VK_DOWN:
            case KeyEvent.VK_S:
            case KeyEvent.VK_LEFT:
            case KeyEvent.VK_A:
            case KeyEvent.VK_RIGHT:
            case KeyEvent.VK_D:
                myTank.setState(tank.STATE_STAND);//松开时坦克处于站立状态
                break;
        }
    }
    private void keyPressedEventLose(int keyCode) {
        //结束游戏
        if(keyCode==KeyEvent.VK_ESCAPE){
            System.exit(0);
        }
        else if(keyCode==KeyEvent.VK_ENTER){
            setGameState(STATE_MENU);
            //很多游戏的操作需要关闭，游戏的某些属性需要重置
            resetGame();
        }
    }
    //重置游戏状态
    private void resetGame(){
        killEnemyCount = 0;
        menuIndex = 0;
        //先让自己的坦克的子弹还回对象池
        myTank.bulletsReturn();
        //销毁自己的坦克
        myTank = null;
        //清空敌人相关资源
        for(tank t:enemies)
        {
            t.bulletsReturn();
        }
        enemies.clear();
        //清空地图资源
        gameMap = null;
    }

    //使坦克开始转方向的函数
    //游戏运行中的按键的处理方法
    private void keyPressedEventRun(int m1) {
        switch (m1) {
            case KeyEvent.VK_UP:
            case KeyEvent.VK_W:
                myTank.setDir(tank.DIR_UP);
                myTank.setState(tank.STATE_MOVE);
                break;
            case KeyEvent.VK_DOWN:
            case KeyEvent.VK_S:
                myTank.setDir(tank.DIR_DOWN);
                myTank.setState(tank.STATE_MOVE);
                break;
            case KeyEvent.VK_LEFT:
            case KeyEvent.VK_A:
                myTank.setDir(tank.DIR_LEFT);
                myTank.setState(tank.STATE_MOVE);
                break;
            case KeyEvent.VK_RIGHT:
            case KeyEvent.VK_D:
                myTank.setDir(tank.DIR_RIGHT);
                myTank.setState(tank.STATE_MOVE);
                break;
            case KeyEvent.VK_SPACE:
                myTank.fire();
                break;
        }
    }

    private void keyPressedEventAbout(int keyCode) {

    }

    private void keyPressedEventHelp(int keyCode) {

    }

    //菜单状态下的按键的处理
    private void keyPressedEventMenu(int m) {
        switch (m) {
            case KeyEvent.VK_UP:
            case KeyEvent.VK_W:
                if (--menuIndex < 0) {
                    menuIndex = MENUS.length - 1;
                }
                break;
            case KeyEvent.VK_DOWN:
            case KeyEvent.VK_S:
                if (++menuIndex > MENUS.length - 1) {
                    menuIndex = 0;
                }
                break;
            case KeyEvent.VK_ENTER:
                startGame(1);
        }
    }
    //开始游戏并加载level关卡信息
    private static void startGame(int level) {
        enemies.clear();
        if(gameMap==null){
            gameMap = new GameMap();
        }
        gameMap.initMap(level);
        MusicUtil.playStart();
        killEnemyCount = 0;
        bornEnemyCount= 0;
        gameState = STATE_RUN;
        //创建坦克对象，敌人的坦克对象
        myTank = new MyTank(FRAME_WIDTH/3, FRAME_HEIGHT-tank.REDIUS, tank.DIR_UP);//DIR_DOWN表示炮筒向下
        //使用一个单独的线程用于控制生产敌人的坦克,每隔5毫秒产生一个坦克
        new Thread() {
            public void run() {
                while (true) {//本关敌人限制和屏幕敌人限制
                    if (LevelInfo.getInstance().getEnemyCount()>bornEnemyCount
                    &&enemies.size()<ENEMY_MAX_COUNT) {
                        tank enemy = EnemyTank.createEnemy();
                        enemies.add(enemy);
                        bornEnemyCount++;
                    }
                    try {
                        Thread.sleep(ENEMY_BORN_INTERVAL);
                    } catch (InterruptedException e) {
                        e.printStackTrace();
                    }
                    //只有在游戏run 状态才创建敌人坦克
                    if (gameState != STATE_RUN) {
                        break;
                    }
                }
            }
        }.start();
    }

    //是frame类的方法,继承下来的方法，该方法负责了所有的绘制的内容，所有需要在
    //屏幕中显示的内容都要在该方法内调用
    //该方法不能主动调用，需调用repaint（）方法回调该方法
    public void update(Graphics g1) {//step2得到图片的画笔,在bufimg上绘画
        Graphics g = bufImg.getGraphics();
        //step3使用图片画笔将所有内容绘制到图片
        g.setFont(GAME_FONT);
        switch (gameState) {
            case STATE_MENU:
                drawMenu(g);break;
            case STATE_HELP:
                drawHelp(g);break;
            case STATE_ABOUT:
                drawAbout(g);break;
            case STATE_RUN:
                drawRun(g);
                break;
            case STATE_LOSE:
                drawLose(g);
                break;
            case STATE_WIN:
                drawWin(g);
                break;
            case STATE_CROSS:
                drawCross(g);
                break;
        }
        //step4使用系统画笔将图片复制到frame上来
        g1.drawImage(bufImg, 0, 0, null);
    }
    //绘制游戏结束的方法
    private void drawLose(Graphics g) {
        //保证只加载一次
        if(overImg == null){
            overImg = MyUtil.clearImage("res/over.jpg");
        }
        //保证图片剧中对齐
        g.setColor(Color.BLACK);
        g.fillRect(0,0,FRAME_WIDTH,FRAME_HEIGHT);

        int imgW=overImg.getWidth(null);
        int imgH=overImg.getWidth(null);//图片的宽和高

        g.drawImage(overImg,FRAME_WIDTH-imgW>>1,FRAME_HEIGHT-imgH>>1,null);
        //添加按键提示信息
        g.setColor(Color.WHITE);
        g.drawString(OVER_STR0,10,FRAME_HEIGHT-20);
        g.drawString(OVER_STR1,FRAME_WIDTH-200,FRAME_HEIGHT-20);
        //失败文字
        g.setColor(Color.WHITE);
        g.drawString("游戏失败！",FRAME_WIDTH/2-30,50);
    }
    //绘制游戏胜利的界面
    private void drawWin(Graphics g){
        //保证只加载一次
        if(overImg == null){
            overImg = MyUtil.clearImage("res/over.jpg");
        }
        //保证图片剧中对齐
        g.setColor(Color.BLACK);
        g.fillRect(0,0,FRAME_WIDTH,FRAME_HEIGHT);

        int imgW=overImg.getWidth(null);
        int imgH=overImg.getWidth(null);//图片的宽和高

        g.drawImage(overImg,FRAME_WIDTH-imgW>>1,FRAME_HEIGHT-imgH>>1,null);
        //添加按键提示信息
        g.setColor(Color.WHITE);
        g.drawString(OVER_STR0,10,FRAME_HEIGHT-20);
        g.drawString(OVER_STR1,FRAME_WIDTH-200,FRAME_HEIGHT-20);
        //通关文字
        g.setColor(Color.WHITE);
        g.drawString("游戏通关！",FRAME_WIDTH/2-30,50);
    }
    //游戏运行状态的绘制内容
    private void drawRun(Graphics g) {
        //绘制黑色背景
        g.setColor(Color.BLACK);
        g.fillRect(0, 0, FRAME_WIDTH, FRAME_HEIGHT);
        //绘制地图的碰撞层
        gameMap.drawBk(g);//先绘制地图再添加敌人
        drawEnemies(g);
        myTank.draw(g);
        //绘制地图的遮挡层
        gameMap.drawCover(g);
        //爆炸效果
        drawExplodes(g);
        bulletCollideTank();//子弹和坦克碰撞的方法，在tank中有具体的实现
        //子弹和所有地图块的碰撞
        bulletAndTankCollideMapTile();
    }

    //绘制所有的敌人的坦克,如果敌人已经死亡，从容其中移除
    private void drawEnemies(Graphics g) {
        for (int i = 0; i < enemies.size(); i++) {
            tank enemy = enemies.get(i);
            if (enemy.isDie()) {
                enemies.remove(i);//arraylist的移除操作
                i--;
                continue;//ArrayList重新判断元素个数
            }
            enemy.draw(g);
        }
    }

        private void drawAbout (Graphics g){

        }

        private void drawHelp (Graphics g){

        }

        //绘制菜单状态下的内容 graphics是画笔对象，直接使用
        //param g画笔对象，系统提供的
        private void drawMenu (Graphics g)
        {
            //绘制黑色背景
            g.setColor(Color.BLACK);
            g.fillRect(0, 0, FRAME_WIDTH, FRAME_HEIGHT);
            final int STR_WIDTH = 76;//设置了字符串的宽度
            int x = FRAME_WIDTH - STR_WIDTH >> 1;
            int y = FRAME_HEIGHT / 3;
            final int DIS = 50;//DIS是行间距
            g.setColor(Color.WHITE);
            for (int i = 0; i < MENUS.length; i++) {
                if (i == menuIndex) {//选中的菜单项的颜色设置为灰色
                    g.setColor(Color.ORANGE);
                } else//其他的为白色
                {
                    g.setColor(Color.WHITE);
                }
                g.drawString(MENUS[i], x, y + DIS * i);
            }
        }
        //使用线程控制刷新率，所以使用死循环
        public void run () {
            while (true) {
                //在此调用repaint，回调update
                repaint();
                try {
                    Thread.sleep(REPAINT_INTERVAL);
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
            }
        }
        private void bulletCollideTank () {
            for (tank enemy : enemies) {
                //我的坦克的子弹和所有敌人的碰撞
                enemy.collideBullets(myTank.getBullets());
            }
            for (tank enemy : enemies) {
                //敌人的坦克的子弹和我的坦克的碰撞
                myTank.collideBullets(enemy.getBullets());
            }
        }
        //所有的子弹和地图块的碰撞
        private void bulletAndTankCollideMapTile(){
            //自己的坦克的子弹和地图块碰撞
            myTank.bulletsCollideMapTiles(gameMap.getTiles());
            //敌人所有的坦克和地图块的碰撞
            for (tank enemy : enemies) {
                enemy.bulletsCollideMapTiles(gameMap.getTiles());
            }
            //我的坦克和地图的碰撞
            if(myTank.isCollideTile(gameMap.getTiles()))
            {
                myTank.back();
            }
            //敌人的坦克和地图的碰撞
            for (tank enemy : enemies) {
                if(enemy.isCollideTile(gameMap.getTiles())){
                    enemy.back();
                }
            }
            //清理所有的销毁的地图块
            gameMap.clearDestoryTile();
        }
        //所有的坦克上的爆炸效果
        private void drawExplodes (Graphics g){
            for (tank enemy : enemies) {
                enemy.drawExplodes(g);
            }
            myTank.drawExplodes(g);
        }

        public static int getGameState() {
        return gameState;
        }

    public static void setGameState(int gameState) {
        GameFrame.gameState = gameState;
    }
    /**
    *是否是最后一关
     * @return
     */
    public static boolean isLastLevel(){
        //当前关卡和总关卡一致
        int currlevel = LevelInfo.getInstance().getLevel();//当前是第几关
        int levelCount = GameInfo.getLevelCount();
        return currlevel==levelCount;
    }
    //判断是否过关
    public static boolean isCrossLevel(){
        //消灭的敌人的数量，和关卡的敌人的数量一致
        return killEnemyCount == LevelInfo.getInstance().getEnemyCount();
    }
    //进入下一关的方法
    public static void nextLevel(){
        startGame(LevelInfo.getInstance().getLevel()+1);
    }
}